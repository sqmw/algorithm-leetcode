...
class Solution:
    def buildTree(self, preorder: List[int], inorder: List[int]) -> Optional[TreeNode]:
        ...
        def sub_is_right(sub_nums: List[int], src_nums: List[int]) -> bool:
            last_index = -1
            for sub_num in sub_nums:
                now_index = src_nums.index(sub_num)
                if not now_index > last_index:
                    return False
                else:
                    last_index = now_index
            return True

        now_inorder_node_seq: List[TreeNode] = self.get_inorder_node_seq(root)
        now_in_order_seq: List[int] = [item.val for item in now_inorder_node_seq]
        # 每次添加一个节点
        while len(now_in_order_seq) != len(inorder):
            now_node = TreeNode(preorder[len(now_inorder_node_seq)])
            for node in now_inorder_node_seq:
                if node.left is None:
                    node.left = now_node
                    now_inorder_node_seq: List[TreeNode] = self.get_inorder_node_seq(root)
                    now_in_order_seq: List[int] = [item.val for item in now_inorder_node_seq]
                    if sub_is_right(now_in_order_seq, inorder):
                        break
                    else:
                        node.left = None
                if node.right is None:
                    ...
                    if sub_is_right(now_in_order_seq, inorder):
                        ...
                        node.right = None
        return root
    def get_inorder_node_seq(self, root: TreeNode) -> List[TreeNode]:
        des_arr: List[TreeNode] = []

        def inorder_traverse(_root: TreeNode):
            if _root is not None:
                inorder_traverse(_root.left)
                des_arr.append(_root)

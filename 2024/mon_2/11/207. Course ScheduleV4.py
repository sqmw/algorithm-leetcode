...
class Solution:
    def canFinish(self, numCourses: int, prerequisites: List[List[int]]) -> bool:
        ...
        def depth_first_search(node: int):
            nonlocal can_finish
            visited_dic[node] = 1  # 1表示正在访问
            for neighbor in graph[node]:
                if visited_dic[neighbor] == 0:
                    depth_first_search(neighbor)
                elif visited_dic[neighbor] == 1:
                    can_finish = False  # 存在环
            visited_dic[node] = 2  # 2表示已经访问完毕
        ...
        # 访问的过程中，顶多只有1个节点被访问，
        visited_dic = {i: 0 for i in range(numCourses)}  # 0表示未访问，1表示正在访问，2表示已经访问完毕
        can_finish = True

        for node in range(numCourses):
            if visited_dic[node] == 0:
...
if __name__ == "__main__":
    ...
    prerequisites = [[1, 0], [2, 0], [2, 1], [3, 1], [3, 2], [4, 2], [4, 3], [5, 3], [5, 4], [6, 4], [6, 5], [7, 5],
                     [7, 6], [8, 6], [8, 7], [9, 7], [9, 8], [10, 8], [10, 9], [11, 9], [11, 10], [12, 10], [12, 11],
